...
function calcDeadline(lt, iso) {
  const d = new Date(iso);
  d.setDate(d.getDate() - lt);
  return d;
}
function daysLeft(lt, iso) {
  return Math.ceil((calcDeadline(lt, iso) - new Date()) / 864e5);
}
...
function getEventLabel(month) {
  return EVENT_LABELS[month] || null;
}
...
function sortProducts(list, targetISO) {
  return [...list].sort((a, b) => {
    const sa = urgencyScore(a, targetISO),
      sb = urgencyScore(b, targetISO);
...
function getCardBadge(r, eventLabel) {
  if (r.b_backorder > 0)
    return { text: "Perlu Perhatian", type: "danger" };
  if (r.e_expres_display && r.e_expres_display !== "-")
    return { text: "Perlu Perhatian", type: "warning" };
  if (r.y_lembur > 0)
    return {
      text: eventLabel ? `Produk Utama ${eventLabel}` : "Prioritas Lembur",
      type: "primary",
    };
...
function RingkasanCard({ label, value, sub, accent }) {
  ...
  return (
    <div
      className={`border-2 ${c} rounded-2xl p-4 hover:scale-[1.02] transition-transform duration-200 cursor-default`}
    >
...
function DeadlineChip({ leadTime, tISO }) {
  ...
  if (dl < 0) {
    ...
    text = `TERLAMBAT ${Math.abs(dl)} hari`;
  } else if (dl === 0) {
    color = "text-red-600 bg-red-50";
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 text-[11px] font-medium px-2 py-0.5 rounded-md ${color}`}
    >
      <Clock size={11} />
      {text}
    </span>
...
function DecBox({
  ...
  return (
    <div
      className={`${s.bg} ${s.bd} border rounded-xl p-3 text-center ${
        highlight ? "ring-2 ring-inset ring-current/20" : ""
      }`}
    >
      <p
        className={`${s.l} text-[10px] uppercase tracking-wider font-semibold`}
      >
        {label}
      </p>
...
function MiniBox({ label, value, varLabel }) {
  return (
    <div className="bg-gray-50 border border-gray-200 rounded-xl p-3 text-center">
      ...
      <p className="text-gray-800 font-bold text-xl leading-tight mt-0.5">
        {value}
      </p>
...
function QuoteBlock({ title, body, footer }) {
  return (
    <div className="border-l-4 border-amber-400 bg-amber-50 rounded-r-xl p-4 space-y-1.5">
      ...
      {footer && (
        <p className="text-[12px] text-gray-500 italic">{footer}</p>
      )}
    </div>
  );
}
  ...
  return (
    <div className="flex items-start gap-4">
      <div className="w-[20%] shrink-0 pt-1">
        <p className="text-[11px] text-gray-500 uppercase tracking-wider font-semibold leading-tight">
          Skenario
          <br />
          permintaan
        </p>
        ...
        {items.map((it) => (
          <div key={it.key} className="flex items-center gap-3">
            <span className="text-[11px] text-gray-500 w-28 shrink-0">
              <span className="font-semibold text-gray-700">{it.key}</span> (
              {it.label})
            </span>
            ...
            <span className="text-[12px] font-semibold text-gray-700 w-14 text-right">
              {it.val} unit
            </span>
          </div>
        ))}
      ...
      <div className="p-4 space-y-4">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          ...
          <DecBox
            ...
            varLabel="y_js"
            accent="amber"
            highlight={r.y_lembur > 0}
          />
          <DecBox
            label="Pengadaan cepat"
            value={r.e_expres_display || "-"}
            varLabel="e_js"
            accent="red"
            highlight={r.e_expres_display === "Pertimbangkan"}
            subLabel={r.e_expres_label}
          />
          <DecBox
            label="Backorder diterima"
            value={r.b_backorder}
            varLabel="b_js"
            accent="neutral"
            highlight={r.b_backorder > 0}
          />
        </div>
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden hover:shadow-md transition-shadow">
      ...
      <div className="p-4 space-y-3">
        ...
        {produk.note && (
          <p className="text-[12.5px] text-gray-500 leading-relaxed pl-0.5">
            {produk.note}
          </p>
        )}
      </div>
...
export default function DashboardView({
  ...
  // Fetch data status saat pertama kali mount
  useEffect(() => {
    if (!dataStatus) {
      fetchDataStatus();
    }
  ...
  // ── Gate logic: apakah data lengkap untuk hitung? ──
  const dataReady =
    dataStatus && dataStatus.has_produk && dataStatus.has_penjualan;
  const missingProduk = dataStatus && !dataStatus.has_produk;
  const missingPenjualan = dataStatus && !dataStatus.has_penjualan;

  // ── Handle tombol hitung (dengan gate) ──
  const handleHitung = () => {
    if (!dataReady) return;
    onRefresh();
  };
  ...
  if (error && !data)
    return (
      <div className="flex flex-col items-center justify-center py-24">
        ...
        <button
          onClick={onRefresh}
          className="px-5 py-2.5 bg-gray-900 text-white font-medium text-sm rounded-xl hover:bg-gray-700 transition-colors"
        >
          Coba Lagi
        </button>
      </div>
  ...
  // ═══════ STATE: BELUM ADA DATA → TAMPILKAN GATE ═══════
  if (!data)
    return (
      <div className="flex flex-col items-center justify-center py-24 max-w-lg mx-auto text-center">
        ...
        <p className="text-sm text-gray-500 mb-6">
          Untuk menjalankan prediksi & optimasi, pastikan data berikut sudah
          lengkap:
        </p>

        {/* Checklist status data */}
        <div className="w-full bg-white border border-gray-200 rounded-2xl p-5 mb-6 text-left space-y-3">
          <div className="flex items-center gap-3">
            <span
              className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${
                dataStatus?.has_produk
        ...
        {/* Tombol Mulai Hitung — disabled kalau data belum lengkap */}
        <button
          onClick={handleHitung}
          disabled={!dataReady}
          className={`px-6 py-3 font-medium text-sm rounded-xl transition-colors ${
            dataReady
        ...
        >
          {dataReady
            ? "Mulai Hitung"
            : "Mulai Hitung — Lengkapi Data Dulu"}
        </button>

        <p className="text-[11px] text-gray-400 mt-3">
          Kedua data di atas menjadi dasar prediksi P10, P50, P90.
        </p>
      </div>
    );

  if (!data.header || !data.produkList)
    return (
      <div className="py-12 text-center">
        <p className="text-red-400 text-sm">Data diterima tapi format salah</p>
        <pre className="text-[10px] text-gray-400 mt-2 max-w-md mx-auto text-left bg-gray-50 p-3 rounded-lg overflow-auto">
  ...
  const totalReg = data.produkList.reduce(
    (s, p) => s + (p.rekomendasi.x_reguler || 0),
    0
  );
  const jamLembur = data.produkList.reduce(
    (s, p) => s + (p.rekomendasi.y_lembur || 0) * (p.waktuJam || 0),
    0
  );
  const jamReg = data.produkList.reduce(
    (s, p) => s + (p.rekomendasi.x_reguler || 0) * (p.waktuJam || 0),
    0
  );
  ...
  const sortedPengadaan = [...(data.pengadaan || [])].sort(
    (a, b) => daysLeft(a.leadTime, tISO) - daysLeft(b.leadTime, tISO)
  );
  ...
  return (
    <div className="space-y-8">
      ...
      <header className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4 pb-6 border-b border-gray-200 anim-in">
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-500 mt-1">
            {data.header.umkm} · {gLabel} ({hLabel})
          </p>
          <div className="flex items-center gap-2 mt-3 flex-wrap">
            <span className="text-[11px] font-semibold tracking-wide uppercase bg-gray-100 text-gray-600 px-3 py-1 rounded-full">
              {data.header.meta}
            </span>
            {eventLabel && (
              <span className="text-[11px] font-semibold tracking-wide bg-amber-50 text-amber-700 border border-amber-200 px-3 py-1 rounded-full">
                🌙 {eventLabel}
              </span>
            )}
            <button
              onClick={() => setMonthOpen(true)}
              className="text-[11px] font-semibold tracking-wide bg-blue-50 text-blue-600 border border-blue-200 px-3 py-1 rounded-full hover:bg-blue-100 transition-colors"
            >
              {gLabel} ▾
            </button>
          </div>
        ...
        <div className="flex items-center gap-2 self-start sm:self-auto flex-wrap">
          <button
            ...
            className="flex items-center gap-1.5 px-3.5 py-2.5 bg-white border border-gray-200 text-gray-700 text-sm font-medium rounded-xl hover:bg-gray-50 transition-colors"
          >
            <Upload size={15} />Input Data
          </button>
          <button
            onClick={handlePdf}
            className="flex items-center gap-1.5 px-3.5 py-2.5 bg-white border border-gray-200 text-gray-700 text-sm font-medium rounded-xl hover:bg-gray-50 transition-colors"
          ...
          <button
            onClick={handleHitung}
            disabled={refreshing}
            className="flex items-center gap-1.5 bg-gray-900 text-white px-4 py-2.5 rounded-xl text-sm font-medium hover:bg-gray-700 disabled:bg-gray-400 transition-colors"
          >
            {refreshing ? (
      ...
      {monthOpen && (
        ...
        >
          <div
            className="absolute top-32 left-1/2 -translate-x-1/2 bg-white rounded-xl shadow-xl border border-gray-200 p-4 w-72 modal-anim"
              ...
              {BULAN.map((b, i) => {
                ...
                return (
                  <button
                    key={b}
                    onClick={() => {
                      setTargetDate(new Date(d.getFullYear(), i, 1));
                      setMonthOpen(false);
                    }}
      ...
      {/* ═══════ RINGKASAN ═══════ */}
      <section>
        <h2 className="text-[11px] uppercase tracking-[0.15em] text-gray-400 font-semibold mb-4 anim-in anim-d1">
          Ringkasan Bulan Ini
        </h2>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4">
          <RingkasanCard
            label="Estimasi keuntungan"
            value={data.estimasiKeuntungan}
            sub="Expected profit optimal"
            accent="emerald"
          />
          <RingkasanCard
            label="Total produksi reguler"
            value={`${totalReg} unit`}
            sub={`dari ${data.produkList.length} jenis produk`}
            accent="blue"
          />
          <RingkasanCard
            label="Jam lembur dibutuhkan"
            value={`${Math.round(jamLembur)} jam`}
            sub={`dari maks ${data.kapasitas.lembur_maks} jam/bulan`}
            accent="amber"
          />
          <RingkasanCard
            ...
            sub="reguler + lembur"
            accent={
              utilisasi > 90 ? "amber" : utilisasi > 70 ? "blue" : "neutral"
            }
          />
        </div>
      </section>
      ...
      <section className="space-y-4">
        <h2 className="text-[11px] uppercase tracking-[0.15em] text-gray-400 font-semibold mb-4 anim-in anim-d2">
          Rencana Produksi Per Produk
        </h2>
        {fullCards[0] && (
      ...
      <section className="anim-in anim-d5">
        <div className="flex items-baseline justify-between mb-4">
          <h2 className="text-[11px] uppercase tracking-[0.15em] text-gray-400 font-semibold">
            Jadwal Pengadaan Bahan Baku
          </h2>
          <p className="text-[12px] text-gray-500">
            Apa yang harus dipesan dan kapan
          </p>
        </div>
        <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden divide-y divide-gray-100">
          {sortedPengadaan.map((item) => {
            ...
            let txt;
            if (isLocal)
              txt = "Tersedia lokal, bisa pesan kapan saja";
            else if (dl < 0)
              txt = `Pesan sekarang — terlambat ${Math.abs(dl)} hari`;
            else if (dl <= 3)
              txt = `Pesan sekarang — lead time ${item.leadTime} hari`;
            else
              txt = `Pesan paling lambat ${fmtShort(
                calcDeadline(item.leadTime, tISO)
              )}`;
            return (
              <div
                key={item.material}
                className={`flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-0 px-5 py-3.5 ${
                  isUrgent ? "bg-red-50/40" : ""
                }`}
              >
                <span className="sm:w-[40%] font-medium text-[13px] text-gray-800 shrink-0">
                  {item.material}
                </span>
                <span className="sm:w-[20%] shrink-0">
                  <Badge type={isUrgent ? "danger" : "safe"}>
                    {isUrgent ? "Ekspres" : "Reguler"}
                  </Badge>
                </span>
                <span
                  className={`sm:w-[40%] text-[13px] ${
                    isUrgent
      ...
      <footer className="text-[12px] text-gray-400 border-t border-gray-200 pt-5 pb-10 leading-relaxed anim-in anim-d6">
        <p>
          Rekomendasi dihasilkan dari optimasi TSSP-PHA atas 5 skenario
          permintaan musiman. Angka produksi{" "}
          <span className="text-gray-600 font-semibold">bukan</span> sekadar
          mengikuti forecast — melainkan solusi yang memaksimalkan expected
          profit. Kapasitas:{" "}
          <span className="text-gray-600 font-semibold">
            {data.kapasitas.reguler_maks} jam reguler +{" "}
            {data.kapasitas.lembur_maks} jam lembur
          </span>
          .
        </p>
      </footer>
