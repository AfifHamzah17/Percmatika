 ...
 *   item.waktu_jam                      → waktuJam  (ditambahkan di api.py /api/hitung)
...
const getMaterial = (nama) => MATERIAL_MAP[nama] ?? `Bahan ${nama.toLowerCase()}`;

// ── Map e_ekspres_label → nilai yang dipakai DashboardView ─────────
// View memeriksa: e_expres_display !== "-" → tampilkan warning
//                 e_expres_display === "Pertimbangkan" → badge merah
function mapEkspresDisplay(ekLabel) {
  if (!ekLabel || ekLabel === "Tidak") return "-";
  // "X unit(P50)/Y unit(peak)" → ada ekspres → tampilkan sebagai "Pertimbangkan"
...
const MOCK_DATA = {
  ...
  produkList: [
    {
      nama: "Sarung Bantal", waktuJam: 3.2, marginJam: 78750, leadTime: 14,
      estimasiProfit: "Rp 0.59 jt",
      forecast: { P10: 5, P50: 9, P90: 14 },
      rekomendasi: { x_reguler: 7, y_lembur: 2, e_expres_display: "-", e_expres_label: "Tidak", b_backorder: 1 },
      quote: { title: "Mengapa ada lembur?", body: "Sarung Bantal memiliki margin tertinggi per jam (Rp 78.750). Model memprioritaskan kapasitas lembur ke produk ini." },
    },
    {
      nama: "Sajadah", waktuJam: 19.2, marginJam: 3750, leadTime: 10,
      estimasiProfit: "Rp 0.09 jt",
      forecast: { P10: 5, P50: 8, P90: 12 },
      rekomendasi: { x_reguler: 7, y_lembur: 0, e_expres_display: "Pertimbangkan", e_expres_label: "1 unit(P50)/3 unit(peak)", b_backorder: 2 },
      quote: { title: "Pengadaan cepat tersedia", body: "Demand sajadah meningkat saat Ramadhan. Pengadaan ekspres diaktifkan untuk skenario peak." },
    },
    {
      nama: "Totebag", waktuJam: 19.2, marginJam: 7500, leadTime: 2,
      estimasiProfit: "Rp 0.26 jt",
      forecast: { P10: 5, P50: 8, P90: 11 },
      rekomendasi: { x_reguler: 6, y_lembur: 0, e_expres_display: "-", e_expres_label: "Tidak", b_backorder: 2 },
      note: "Demand P50:8. Reguler 6 mencukupi. Lead time 2 hari — bisa pesan mendekati tanggal produksi.",
    },
    {
      nama: "Taplak Meja", waktuJam: 12.8, marginJam: 4500, leadTime: 30,
      estimasiProfit: "Rp 0.27 jt",
      forecast: { P10: 6, P50: 9, P90: 10 },
      rekomendasi: { x_reguler: 7, y_lembur: 0, e_expres_display: "-", e_expres_label: "Tidak", b_backorder: 0 },
      note: "Lead time 30 hari — harus pesan bahan paling awal. Reguler 7 mencukupi.",
    },
    {
      nama: "Pouch", waktuJam: 6.4, marginJam: 28125, leadTime: 4,
      estimasiProfit: "Rp 0.19 jt",
      forecast: { P10: 3, P50: 7, P90: 12 },
      rekomendasi: { x_reguler: 7, y_lembur: 2, e_expres_display: "-", e_expres_label: "Tidak", b_backorder: 0 },
      note: "Demand P50:7. Reg7+Lem2 mencukupi. Margin/jam Rp28.125 ≥ median.",
    },
    {
      nama: "Selimut", waktuJam: 44.8, marginJam: 8035, leadTime: 7,
      estimasiProfit: "Rp 0.60 jt",
      forecast: { P10: 5, P50: 6, P90: 8 },
      rekomendasi: { x_reguler: 5, y_lembur: 0, e_expres_display: "-", e_expres_label: "Tidak", b_backorder: 1 },
...
export function transformApiResponse(raw, targetISO) {
  // Guard: validasi struktur minimal
  ...
  // Hitung total margin weight untuk estimasi profit proporsional
  let totalWeight = 0;
  ...
  const produkList = tssp.alokasi_produk.map((item, i) => {
    ...
    const isUrgent = (rek.y_lembur_max ?? 0) > 0
                  || (rek.b_backorder_max ?? 0) > 0
                  || (rek.e_ekspres_label && rek.e_ekspres_label !== "Tidak");

    // Estimasi profit proporsional berdasarkan margin × volume
    const profitShare = totalWeight > 0 ? (weights[i] / totalWeight) * tp : 0;

    // e_ekspres: map ke format yang dipakai View
  ...
  // Jadwal pengadaan dari backend, diurutkan lead time terpanjang dulu
  ...
  // Pakai targetISO dari arg (bukan dari raw) agar sinkron dengan AppContext
  const usedISO = targetISO ?? new Date().toISOString();
  const event   = raw.event ?? "normal";
  const eventMap = {
    ramadhan: "Ramadhan", pra_ramadhan: "Pra-Ramadhan", syawal: "Syawal",
    dzulhijjah: "Idul Adha", natal: "Natal", tahun_ajaran: "Tahun Ajaran",
  };
...
// ── useDashboardModel — hook yang dipakai DashboardPresenter ────────
export function useDashboardModel() {
  const { targetDate, umkm, produkList, dashboardData, setDashboardData } = useApp();

  // INSTANT: pakai data dari context HANYA kalau bulannya cocok dengan targetDate sekarang.
  // Kalau bulan beda → mulai dari null (nanti cek cache, kalau tidak ada → tampilkan prompt).
  ...
  dataRef.current = data; // selalu up-to-date tanpa memicu re-render
  ...
  // Dipanggil sekali saat mount, dan saat user ganti bulan via picker.
  // Kalau cache ada → tampilkan. Kalau tidak ada → setData(null) supaya
  // view menampilkan prompt "Klik Hitung". TIDAK pernah memicu modal.
  const checkCacheOnly = useCallback(async () => {
    const bulan = _bulanTarget();
    // Sudah ada data untuk bulan ini → skip total
    ...
    try {
      ...
      // Cache tidak ada → reset data lokal, view akan tampilkan prompt "Mulai Hitung"
      setData(null);
    } finally {
  ...
  // ── Hitung ulang — HANYA dipanggil saat user klik tombol "Hitung" ─
  const runOptimization = async () => {
    ...
    try {
      ...
      if (transformed) {
        toast.success(`Optimasi selesai! Expected profit: ${transformed.estimasiKeuntungan}`, { position: "top-right" });
      }
    } catch (err) {
      if (err.name === "AbortError") return;
      console.warn("[Dashboard] API error, tampilkan mock data:", err.message);
      const mock = { ...MOCK_DATA, header: { ...MOCK_DATA.header, targetDate: targetDate.toISOString(), umkm: umkm.nama } };
      setData(mock);
      setDashboardData(mock);
      toast.info("Data contoh ditampilkan (API belum tersedia)", { position: "top-right" });
    } finally {
      if (!ctrl.signal.aborted) setIsComputing(false);
    }
  };

  return { data, isCheckingCache, isComputing, isLoading: isCheckingCache || isComputing, error, runOptimization, checkCacheOnly };
}